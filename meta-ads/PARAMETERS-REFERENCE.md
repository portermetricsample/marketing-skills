...
⚠️ **`bid_strategy` va DONDE está el presupuesto:** en el `campaign_create` **solo si la campaña es CBO** (presupuesto en la campaña). En una campaña **no-CBO**, pasarlo en el `campaign_create` falla con `subcode 1885737 "No Budget for Campaign"` — ahí la puja va en el **ad set** (`adset_create.bid_strategy`). Validado 2026-07-16.
...
- `daily_budget_amount` / `lifetime_budget_amount` — ⚠️ **unidades MÍNIMAS (centavos)** en AMBOS niveles: TÚ multiplicas × offset; el connector **NO** convierte (verificado 2026-07-16 con read-back — el schema dice "unidad mayor/convierte" pero es **FALSO**). `30.000 COP/día` = `3000000`. Siempre vía `campaigns/_budget/budget.md` + read-back.
...
- **Advantage+ audience:** `targeting_advantage_audience` `1` (Meta amplía) / `0` (manual). ⚠️ **Obligatorio enviarlo** (issue #9). ⚠️ Con `1` **no se permite `age_max` < 65** (subcode 1870189) — para un tope de edad real usa `0`.
...
- ❌ **El lead form NO va en el ad set.** Meta rechaza `lead_gen_form_id` dentro del `promoted_object` del ad set (`Invalid keys "lead_gen_form_id"…`, validado 2026-07-16). Para LEADS el promoted_object del ad set es **solo la Página** (`promoted_object_page_id`); el lead form se adjunta en el **anuncio** (`ad_create.lead_gen_form_id`, §3).
...
- `link` — URL de destino. ⚠️ **Obligatoria en TODO anuncio con imagen/link, incluido LEADS** (subcode 2061015 si falta), no solo TRAFFIC/SALES.
...
- **Video simple:** `video_id` (+ `image_hash` opcional = thumbnail). ⚠️ **`description` NO se soporta en video** (subcode 1443050); solo `message`/`headline`/`cta`/`link`.
