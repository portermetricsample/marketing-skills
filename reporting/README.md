...
| [`porter-linkedin-pages-dashboard/`](porter-linkedin-pages-dashboard/) | A specific, ready-made 4-page LinkedIn Company Page dashboard (Overview · Posts · Audience · Discovery), shipped as bundled report source so every run lands on the same design. Runtime-resolved account, so the clone path reproduces it in one MCP call — no build tools, works in plain chat | ✅ Built (published + clone verified) |
